...
public class StatsMapper implements Mapper<GameState, StatsEntity> {
    ...
    public GameState toDomain(StatsEntity entity) {
        return new GameState(entity.counter(), entity.clicksPerSecond(), entity.counterPerClick(), entity.purchasedBuildings());
    }

    @Override
    public StatsEntity toEntity(GameState domain) {
        return new StatsEntity(domain.getCounter(), domain.getClicksPerSecond(), domain.getCounterPerClick(), domain.getPurchasedBuildings());
    }
